...
void SetGamepadDisplayIMUValues(GamepadDisplay *ctx, float *gyro_drift_solution, float *euler_displacement_angles, int reported_senor_rate_hz, int estimated_sensor_rate_hz, float drift_calibration_progress_frac)
{
    if (!ctx) {
        return;
    }
                ...
                */
                ...
                /* Display rate of gyro as provided by the HID implementation. This could be based on a hardware time stamp (PS5), or it could be generated by the HID implementation. */
                {
                    if (ctx->reported_sensor_rate_hz > 0)
                    {
                        ...
                        const Uint16 deltatime_ms = 1000 / ctx->reported_sensor_rate_hz;
                        SDL_strlcpy(text, "Sensor Time:", sizeof(text));
                        SDLTest_DrawString(ctx->renderer, x + center - SDL_strlen(text) * FONT_CHARACTER_SIZE, y, text);
                        SDL_snprintf(text, sizeof(text), "%dms %dhz", deltatime_ms, ctx->reported_sensor_rate_hz);
                        SDLTest_DrawString(ctx->renderer, x + center + 2.0f, y, text);
                    }
                }
                ...
                /* Display the instrumentation's count of all packets recieved over time. This may represent a more accurate polling rate for the IMU */
                {
                    if (ctx->estimated_sensor_rate_hz > 0)
                    {
                        y += ctx->button_height + 2.0f;
                        /* Convert ms to seconds */
                        Uint16 deltatime_ms =  1000 / ctx->estimated_sensor_rate_hz ;
                        SDL_strlcpy(text, "Est.Sensor Time:", sizeof(text));
                        SDLTest_DrawString(ctx->renderer, x + center - SDL_strlen(text) * FONT_CHARACTER_SIZE, y, text);
                        SDL_snprintf(text, sizeof(text), "%dms %dhz", deltatime_ms, ctx->estimated_sensor_rate_hz);
                        SDLTest_DrawString(ctx->renderer, x + center + 2.0f, y, text);
                    }
                }
